		...
		for event in longpoll.listen():
		    ...
		    if event.type == VkBotEventType.MESSAGE_NEW:
		    	...
		    	if event.from_chat and event.message and event.message.get('text'):  #ЕСЛИ НАПИСАЛИ В ЧАТЕ
	    			...
	    			elif "баланс💰" in reseived_message.lower():
		    		...
		    		#elif "Получить финансовую помощь💰" in reseived_message.lower():



		    		#КОМАНДЫ НУЖНО ПИСАТЬ ТОЛЬКО С ОБРАЩЕНИЕМ К БОТУ

		    		elif ("едвард " in reseived_message.lower() or "эдвард " in reseived_message.lower() or "бот " in reseived_message.lower() or "edward " in reseived_message.lower() or "EDWARD | BOT " in reseived_message.lower()) or ("едвард, " in reseived_message.lower() or "эдвард, " in reseived_message.lower() or "бот, " in reseived_message.lower() or "edward, " in reseived_message.lower()):    #Проверяем, обратились ли к боту
		    		...
		    		#КОМАНДЫ МОЖНО ПИСАТЬ, БЕЗ ОБРАЩЕНИЯ К БОТУ
				    	...
				    	elif index == "баланс" or index == "баланс💰":
				    		iduser = event.message.get("from_id")
			    			user = vk_session.method("users.get", {"user_ids": iduser})
			    			name = user[0]['first_name']
		    		...
		    		if reseived_message.lower() in hello_list_user:
		    		...
		    		elif reseived_message.lower() == "/бонус": #БОНУС
		    			f = open('Bonus.txt', 'r+')
		    			balance = f.readlines()
		    			id_user = False
		    			iduser = event.message.get("from_id")
		    			...
		    			if id_user == False:
		    				...
		    				if id_user2 == False:
		    					...
		    					f.write(balance_new)
		    					f.close()
		    					a = name + ", " + "на вашем счету сейчас: " + str(1050000) + "💲"
		    					write_balance2(sender, a)
