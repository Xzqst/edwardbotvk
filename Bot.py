		...
		for event in longpoll.listen():
		    ...
		    if event.type == VkBotEventType.MESSAGE_NEW:
		    	...
		    	if event.from_chat and event.message and event.message.get('text'):  #ЕСЛИ НАПИСАЛИ В ЧАТЕ
		    		...
		    		#КОМАНДЫ МОЖНО ПИСАТЬ, БЕЗ ОБРАЩЕНИЯ К БОТУ
		    			...
		    			if index == "эдвард" or index == "едвард" or index == "бот" or index == "edward":
			    			...
			    			try:
				    			...
				    			write_message(sender, "⚠Напишите команду правильно!")

			    		elif "трейд " in index: # ИГРА ТРЕЙД
			    			try:
					    		iduser = event.message.get("from_id")
					    		user = vk_session.method("users.get", {"user_ids": iduser})
					    		name = user[0]['first_name'] #получение имени
					    		a = index.split("трейд ", 1)
					    		a = a[1].split(" ", 1)
					    		app_down = a[0] #вверх/вниз
					    		bet = a[1] #ставка
					    		app_down_list = ["вверх", "вниз"]
					    		course = [1, 5, 10, 50, 34, 133, 36, 73, 81, 100, 121, 41, 32, 93, 423, 523, 123, 543, 832]
					    		r = random.choice(app_down_list)
					    		c = random.choice(course)
					    		f = open('Balance.txt', 'r')
					    		balance = f.readlines()
					    		id_user = False
				    			...
				    			for b in balance:
				    				if str(iduser) in b:
				    					...
				    					if check >= 0:
				    						if app_down == r and r == "вверх":

					    						m1 = "👩‍💼" + name + ", " + "📈курс валюты подорожал на: " + str(c) + "💲"
					    						m2 = "💵Вы выйграли: " + bet + "💲"
					    						balance_new = int(x) + int(bet) #новый баланс игрока
					    						m3 = name + ", " + "ваш баланс💰: " + str(balance_new)
					    						balance_new2 = str(iduser) + " " + str(balance_new)
					    						del balance[index]
					    						balance.append(balance_new2)
					    						f = open('Balance.txt', 'w')
					    						f.writelines(balance)
					    						f.close()
					    						write_message(sender, m1)
					    						write_message(sender, m2)
					    						write_balance(sender, m3)

					    					elif app_down == r and r == "вниз":

					    						m1 = "👩‍💼" + name + ", " + "📉курс валюты подешевел на: " + str(c) + "💲"
					    						m2 = "💵Вы выйграли: " + bet + "💲"
					    						balance_new = int(x) + int(bet) #новый баланс игрока
					    						m3 = name + ", " + "ваш баланс💰: " + str(balance_new)
					    						balance_new2 = str(iduser) + " " + str(balance_new)
					    					...
					    					elif not app_down == r and r == "вверх":
					    						m1 = "👩‍💼" + name + ", " + "📈курс валюты подорожал на: " + str(c) + "💲"
					    						m2 = "💵Вы проиграли: " + bet + "💲"
					    						balance_new = int(x) - int(bet) #новый баланс игрока
					    						m3 = name + ", " + "ваш баланс💰: " + str(balance_new)
					    					...
					    					elif not app_down == r and r == "вниз":
					    						m1 = "👩‍💼" + name + ", " + "📉курс валюты подешевел на: " + str(c) + "💲"
					    						m2 = "💵Вы проиграли: " + bet + "💲"
					    						balance_new = int(x) - int(bet) #новый баланс игрока
					    						m3 = name + ", " + "ваш баланс💰: " + str(balance_new)
					    		...
					    		if id_user == False:
					    			...
					    			if check >= 0:
				    					if app_down == r and r == "вверх":
				    						...
				    						m1 = "👩‍💼" + name + ", " + "📈курс валюты подорожал на: " + str(c) + "💲"
				    						m2 = "💵Вы выйграли: " + bet + "💲"
				    						balance_new = start_balance + int(bet) #новый баланс игрока
				    						m3 = name + ", " + "ваш баланс💰: " + str(balance_new)
				    						balance_new2 = "\n" + str(iduser) + " " + str(balance_new)
				    						f = open('Balance.txt', 'r+')
				    						f.write(balance_new2)
					    		...
					    		write_message(sender, "⚠Что-то пошло не так. Введите команду правильно. Например: /трейд вверх 100")
		    		...
		    		elif "/трейд " in reseived_message.lower(): # ИГРА ТРЕЙД
		    			try:
				    		iduser = event.message.get("from_id")
				    		user = vk_session.method("users.get", {"user_ids": iduser})
				    		name = user[0]['first_name'] #получение имени
				    		a = reseived_message.lower().split("трейд ", 1)
				    		a = a[1].split(" ", 1)
				    		app_down = a[0] #вверх/вниз
				    		bet = a[1] #ставка
				    		app_down_list = ["вверх", "вниз"]
			    				...
			    				if str(iduser) in b:
			    					...
			    					if check >= 0:
			    						if app_down == r and r == "вверх":
				    						...
				    						f.writelines(balance)
				    						f.close()
				    						write_message2(sender, m1)
				    						write_message2(sender, m2)
				    						write_balance2(sender, m3)
				    		...
				    		write_message2(sender, "⚠Что-то пошло не так. Введите команду правильно. Например: /трейд вверх 100")
