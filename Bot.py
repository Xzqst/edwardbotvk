...
import Tanos_script as tanos
import Report_script as report
...
def execute(event): #Выполнение команд в БЕСЕДЕ
	...
	if reseived_message.lower()[0] == "/":
		...
		surname = get_user[0]["last_name"]
		text = reseived_message.lower()

		if not user == 278339864:
			vk_session.method('messages.send', {'chat_id': 25, 'random_id': get_random_id(), 'message': "Кто-то написал в беседе!"})
		...
		elif text[0:3] == "кто" or text[0:6] == "список" or text == "онлайн": #КОМАНДЫ С УЧАСТНИКАМИ БЕСЕДЫ
		...
		elif text[0:6] == "щелчок": #ЩЕЛЧОК ТАНОСА
			tanos.start(sender, text, send, user)

		elif text[0:6] == "репорт": #РЕПОРТ
			report.start(sender, text, send, user, name, surname)
...
def execute2(event): #Выполнение команд в ЛС
	#try:
	reseived_message = event.message.get("text")
	reseived_message = reseived_message.lower()
	...
	if reseived_message.lower() in cmd.helps or reseived_message == "Начать": #ВСЕ КОМАНДЫ
	...
	elif reseived_message.lower()[0:3] == "кто" or reseived_message.lower()[0:6] == "список" or reseived_message == "онлайн": #КОМАНДЫ ДЛЯ УЧАСТНИКОВ БЕСЕДЫ
	...
	elif reseived_message[0:6] == "щелчок": #ЩЕЛЧОК ТАНОСА
		tanos.start(sender, reseived_message, send, sender)

	elif reseived_message[0:6] == "репорт": #РЕПОРТ
		report.start(sender, reseived_message, send, sender, name, surname)
